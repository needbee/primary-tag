...
class Core_Tests extends Base\TestCase {
	...
	public function test_i18n() {
		...
		\WP_Mock::onFilter( 'plugin_locale' )->with( 'en_US', 'primarytag' )->reply( 'en_US' );
		\WP_Mock::wpFunction( 'load_textdomain', array(
			'times' => 1,
			'args' => array( 'primarytag', 'lang_dir/primarytag/primarytag-en_US.mo' ),
		) );
		...
		\WP_Mock::wpFunction( 'load_plugin_textdomain', array(
			'times' => 1,
			'args' => array( 'primarytag', false, 'path/languages/' ),
		) );
	...
	public function test_init() {
		// Setup
		\WP_Mock::expectAction( 'primarytag_init' );
